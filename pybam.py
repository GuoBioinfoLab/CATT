...
class read():
    ...
    def __init__(self, f, fields=False, decompressor=False):
        ...
        if fields is not False:
            print(fields)
            if not isinstance(fields, list) or len(fields) is 0:
                raise PybamError(
                    '\n\nFields for the static parser must be provided as a non-empty list. You gave a ' +
                    str(
                        type(fields)) +
                    '\n')
            else:
                for field in fields:
                    if field.startswith('sam') or field.startswith('bam'):
                        ...
                        raise PybamError(
                            ...
                            '" from fields ' +
                            str(fields) +
                            ' does not start with "sam" or "bam" and thus is not an avaliable field for the static parsing.\nPrint "pybam.wat" in interactive python to see available field names with explinations.\n')
        ...
        # First we make a generator that will return chunks of uncompressed
        # data, regardless of how we choose to decompress:
        def generator():
            DEVNULL = open(os.devnull, 'wb')

            # First we need to figure out what sort of file we have - whether
            # it's gzip compressed, uncompressed, or something else entirely!
            if isinstance(f, str):
                try:
                ...
                raise PybamError(
                    '\n\nInput file was not a string or a file object. It was: "' +
                    str(f) +
                    '"\n')
            ...
            if magic == b'BAM\1':
                ...
                # else look for pigz or gzip:
                    ...
                    # Python's gzip module can't read from a stream that
                    # doesn't support seek(), and the zlib module cannot read
                    # the bgzip format without a lot of help:
                    ...
                    while raw_data:
                        ...
                        if magic != b"\x1f\x8b\x08\x04":
                            raise PybamError(
                                '\n\nThe input file is not in a format I understand. First four bytes: ' +
                                repr(magic) +
                                '\n')
                        try:
                            more_bs = bs + \
                            ...
                            bs = more_bs
                        except BaseException:  # zlib doesnt have a nice exception for when things go wrong. just "error"
                            ...
                            raw_data = raw_data[bs +
                                                ...
                                                extra_len -
                                                19]
                            # I have left the numbers in verbose, because the
                            # above try is the optimised code.
                            crc_data = raw_data[bs +
                                                16 +
                                                subfield_len +
                                                block_size -
                                                extra_len -
                            ...
                            # I've left the manual crc checks in for
                ...
                # someone with access to python, so it has system access
                # anyway. The file/data, however, should not be trusted.
                magic_file = os.path.join(tempfile.mkdtemp(), 'magic')
                with open(magic_file, 'wb') as mf:
                    mf.write(magic)
                if isinstance(f, str):
                    self._subprocess = subprocess.Popen(
                        decompressor.replace(
                            '{}',
                            f),
                        shell=True,
                        stdout=subprocess.PIPE,
                    ...
                    self._subprocess = subprocess.Popen(
                        '{ cat "' + magic_file + '"; cat; } | ' + decompressor,
                        stdin=self._file,
                        shell=True,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE)
        ...
        # At this point, we know that whatever decompression method was used, a
        # call to self._generator will return some uncompressed data.
        ...
        while len(header_cache) < p_to:
            header_cache += str(next(self._generator))
        ...
        for _ in range(number_of_reference_sequences):
            ...
            while len(header_cache) < p_to:
                header_cache += next(self._generator)
            self.file_chromosomes.append(
                header_cache[p_from:p_to - 1].decode('ascii'))
            p_from = p_to
            p_to += 4
            while len(header_cache) < p_to:
                header_cache += next(self._generator)
        ...
        def new_entry(header_cache):
            # we keep a small cache of X bytes of decompressed BAM data, to
            # smoothen out disk access.
            cache = header_cache
        ...
        def compile_parser(self, fields):
            ...
            # First we figure out what data from the static portion of the BAM
            # entry we'll need:
            tmp = {}
            ...
            # Fixed-length BAM data (where we just grab the bytes, we dont
            # unpack) can, however, be grabbed individually.
        ...
        while offset != len(self.bam):
            ...
            # exec() compiles "code" to real code, creating the "parser"
            # function and adding it to exec_dict['parser']
            exec(code, exec_dict)
    ...
    # Methods to pull out raw bam data from entry (so still in its binary
    # encoding). This can be helpful in some scenarios.
    @property
    ...
    def sam_qname(self): return self.bam[36: self._end_of_qname - 1].decode()

    @property
    def sam_cigar_list(self): return [(cig >> 4,
                                       cigar_codes[cig & 0b1111]) for cig in array('I',
                                                                                   self.bam[self._end_of_qname: self._end_of_cigar])]
    ...
    @property
    def sam_qual(self):
        return ''.join([chr(quality + 33)
                        for quality in self.bam[self._end_of_seq: self._end_of_qual]])
    ...
    def sam_tags_list(self):
        ...
        while offset != len(self.bam):
            ...
            if tag_type == 'Z':
                ...
                offset_end = offset + 3 + py4py[tag_type]
                tag_data = unpack(CtoPy[tag_type],
                                  self.bam[offset + 3:offset_end])[0]
            elif tag_type == 'B':
                offset_end = offset + 8 + \
                ...
                print(
                    'PYBAM ERROR: I dont know how to parse BAM tags in this format: ',
                    repr(tag_type))
                print(
                    '             This is simply because I never saw this kind of tag during development.')
                print(
                    '             If you could mail the following chunk of text to john at john.uk.com, ill fix this up :)')
                print(repr(tag_type), repr(self.bam[offset + 3:end]))
                exit()
    ...
    def sam_tags_string(self):
        return '\t'.join(A + ':' + ('i' if B in 'cCsSI' else B) + ':' + ((C.typecode + ',' + ','.join(
            map(str, C))) if isinstance(C, array) else str(C)) for A, B, C in self.sam_tags_list)
    ...
    # Internal methods - methods used to calculate where variable-length
    # blocks start/end
    @property
